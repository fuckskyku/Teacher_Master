  ...
  >
    <div class="container">
      <el-scrollbar :native="false" tag="section">
        ...
        <a
          v-if="lookDataObj?lookDataObj.attachUrl:''"
          :href="lookDataObj.attachUrl"
          target="_blank"
        >{{lookDataObj.attachUrl}}</a>
    ...
    <div class="operate" v-if="flag">
      ...
      <el-input v-if="radio==3" placeholder="请输入拒绝的理由（选填）" v-model="reason" clearable></el-input>
    </div>
      ...
      if (this.lookDataObj) {
        if (this.radio) {
          approvalAudit({
            ...
            remark: this.radio == 3 ? this.reason : null
          }).then(res => {
            console.log(res);
