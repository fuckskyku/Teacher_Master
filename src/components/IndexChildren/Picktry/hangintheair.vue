<template>
  <div class="hangintheair">
    ...
    <div class="hangintheair" v-if="parent">
      ...
      <div class="text_group">
        <div class="first-child">
          竟然还有
          <span>{{tobeDone.number}}</span> 个家长未激活
        </div>
    ...
    <el-dialog title="提示" :visible.sync="centerDialogVisible" width="20%" center>
      <span>我们将以短信形式通知未激活的家长激活，是否马上通知？
        <div style="color:red;margin-top:20px;">(注：每24h可以通知一次)</div>
      </span>
    ...
    <!--  发送成绩 -->
    <div class="hangintheair" v-if="TransmissionScoreShow">
      <!-- <div> -->
      <img src="/static/Sroce.png" alt>
      <!-- </div> -->
      <div class="text_group">
        <div class="first-child">还没有发布成绩呢
          <!-- 有
          <span>{{toScore.number}}</span> 篇文章待审批-->
        </div>
        <div class="last-child">马上去发布吧～</div>
      </div>
      <div class="button_group">
        <el-button @click="goScore()">{{toScore.buttonName}}</el-button>
        <el-button @click="skip(1)">略过</el-button>
      </div>
    </div>
    ...
    <div class="hangintheair" v-if="article">
      ...
      <div class="text_group">
        <div class="first-child">
          有
          <span>{{toArticle.number}}</span> 篇文章待审批
        </div>
    ...
    <div class="hangintheair" v-if="copyInfo">
      ...
      <div class="text_group">
        <div class="first-child">
          有
          <span>{{toCopyInfo.number}}</span> 个新的抄送还未读哦
        </div>
...
export default {
    ...
    skip(eventData) {
      ...
      dogin(eventData).then(res => {
        if (res.type == 1) {
          ...
          this.TransmissionScoreShow = false;
          this.end = false;
          this.tobeDone = res;
        } else if (res.type == 2) {
          this.parent = false;
  ...
  created() {
    ...
    if (IsHeadTeacer == 0) {
      ...
      dogin(0).then(res => {
        if (res.type == 0) {
          this.TransmissionScoreShow = true;
          this.toScore = res;
          // alert(1);
        } else if (res.type == 1) {
          this.parent = true;
