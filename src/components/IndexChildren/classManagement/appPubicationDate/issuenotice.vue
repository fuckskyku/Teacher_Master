<template>
  <div id="issuenotice">
      ...
      <el-tab-pane label="已发布" name="issued">
        <div class="seach">
          <div class="seach_left">
            <el-input placeholder="搜索标题、发布者(回车进行搜索)" suffix-icon="el-icon-search" @change="keyWordsInput($event,2)" v-model="keywordsVal" clearable></el-input>
            <div class="section">
              ...
              <el-select v-model="noticeVal" placeholder="通知类型" @change="noticeEvent($event,2)" clearable>
        ...
        <pageHtml :tabObj.sync='tableData' name="schoolNoticePage" :type="examineTypeVal" :typeTwo='noticeVal' :searchsKey='keywordsVal'></pageHtml>
      </el-tab-pane>

      <el-tab-pane label="草稿箱" name="rough">
        <div class="seach">
          <div class="seach_left">
            <el-input placeholder="搜索标题、发布者(回车进行搜索)" suffix-icon="el-icon-search" @change="keyWordsInput($event,1)" v-model="keywordsRoughVal"></el-input>
            <div class="section">
              <div class="section_title">筛选：</div>
              <!-- <el-select v-model="examineTypeRoughVal" placeholder="审核状态" @change="examineEvent($event,1)" clearable>
                <el-option v-for="item in examineTypeObj" :key="item.dictCode" :label="item.dictName" :value="item.dictCode"></el-option>
              </el-select> -->
              <el-select v-model="noticeRoughVal" placeholder="通知类型" @change="noticeEvent($event,1)" clearable>
                <el-option v-for="item in noticeObj" :key="item.dictCode" :label="item.dictName" :value="item.dictCode"></el-option>
              </el-select>
            </div>
          </div>
        ...
        <pageHtml :tabObj.sync='roughList' name="schoolNoticePage" :type="examineTypeRoughVal" :typeTwo='noticeRoughVal' :searchsKey='keywordsRoughVal'></pageHtml>
    ...
    <!-- 通知 -->
    <el-dialog :visible.sync="dialogTableVisibleone" :fullscreen="false" width="580px" top="30vh" :show-close=false :close-on-click-modal="false">
      <div class="notice">
        <div>
