<template>
  <section id="userInfo">
    <div class="absoluteImg">
      <el-upload class="avatar-uploader" action="/api/storage/uploadSingleFile" :multiple="false" :show-file-list="false" :headers="headers" :on-success="handleAvatarSuccessImgSuccess" :before-upload="beforeAvatarUploadImg">

        <img v-if="avatarImg" :src="this.imgJoInUrl() + userInfo.avatarImg" class="avatar">
    ...
    <div class="container">
      <div class="containerInfo">
        <div class="teacherName">教师姓名:
          <span>{{userInfo.realName}}</span>
        </div>
        <div class="six">性别:
          <span>{{userInfo.sex==0?"男":"女"}}</span>
        </div>
        <div class="idCard">身份证号码:
    ...
    <div class="schedule">
      ...
      <div class="backgorundImg">
        <ul class="five">
          <li v-for="item in fivePoint" v-html="item"><span>{{item}}</span></li>
          <li><img src="/static/img/礼物.png" alt=""></li>
        </ul>
        <ul class="six">
          <li><img src="/static/img/礼物.png" alt=""></li>
          <li v-for="item in sixPoint" v-html="item"><span>{{item}}</span></li>
        </ul>
        <ul class="eight">
          <li v-for="item in eightPoint" v-html="item"><span>{{item}}</span></li>
          <li><img src="/static/img/礼物.png" alt=""></li>
        </ul>
        <ul class="thirteen">
          <li><img src="/static/img/礼物.png" alt=""></li>
          <li v-for="item in thirteen" v-html="item"><span>{{item}}</span></li>
        </ul>
      </div>
    </div>

    <!-- 修改手机号码模态框 -->
    <el-dialog :visible.sync="dialogVisibleMobile" :before-close="handleClose" class="reviseMobile" :fullscreen='false' center :inline-message="true" status-icon width="40%">
      <div slot="title" class="reviseTitle">修改手机号码</div>
      <div class="reviseTip">
        更换手机后，下次登录使用新手机号登录，密码不变。 当前手机号：{{userInfo.mobile}}
      </div>

      <el-form class="reviseContent" label-position="right" label-width="100px" :model="reviseInfo" ref="reviseInfo" :rules="isreviseInfo">
        ...
        <el-form-item label="短信验证码:" prop="messageCode" required>
          ...
          <el-col :sm="14" :md="12">
            <el-form-item>
              <el-button type="primary" @click="gainCode(1)" :disabled='isdisabled'>{{isdisabled?smsNum+'秒':'获取短信验证码'}}</el-button>
            </el-form-item>
          </el-col>
        </el-form-item>
    ...
    <!-- 修改密码模态框 -->
    <el-dialog :visible.sync="dialogVisiblePw" :before-close="handleClose" class="reviseMobile" :fullscreen='false' center :inline-message="true" :close-on-click-modal="true" status-icon width="40%">
      <div slot="title" class="reviseTitle">修改密码</div>
      <el-form class="reviseContent" label-position="right" label-width="100px" :model="reviseInfoPw" ref="reviseInfoPw" :rules="isreviseInfoTwo">
        <el-form-item label="手机号码:" prop="mobile">
    ...
    <!-- 修改身份证号码 -->
    <el-dialog :visible.sync="dialogVisibleCard" :before-close="handleClose" class="reviseMobile" :fullscreen='false' center :inline-message="true" :close-on-click-modal="true" status-icon width="40%">
      <div slot="title" class="reviseTitle">修改身份证号码</div>
      <el-form class="reviseContent" label-position="right" label-width="100px" :model="reviseInfoCard" ref="reviseInfoCard" :rules="isreviseInfoCard">
