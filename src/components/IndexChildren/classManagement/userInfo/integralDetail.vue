  ...
  <div id="integralDetail">
    ...
    <div class="header">
      <div class="header_left">
        ...
        <div class="number">{{bonusPoint}}<span>积分</span></div>
        <el-button type="text" class="rule" @click="dialogVisibleRule=true">签到规则</el-button>
    ...
    <el-tabs v-model="activeName" class="tabs" >
      ...
      <el-tab-pane name="recordOfConversion" class="tabsOne">
        <span slot="label"><i class="el-icon-date"></i> 积分详情</span>
        <el-table :data="tableData.data" style="width: 100%" height="520px" :header-cell-style="{textAlign:'center'}" :cell-style="{textAlign:'center'}">
          <!-- <el-table-column type="index"></el-table-column> -->
          <el-table-column prop="changeName" label="项目"> </el-table-column>
          <el-table-column label="日期">
            <template slot-scope="scope">
              <i class="el-icon-time"></i>
              <span style="margin-left: 10px">{{ new Date(scope.row.createTime).format('yyyy-MM-dd hh:mm')}}</span>
            </template>
          </el-table-column>
      ...
      <el-tab-pane name="IntegralDetails" class="tabsTwo">
        <span slot="label"><i class="el-icon-date"></i> 兑换记录</span>
        <el-table :data="tableDataTwo.data" style="width: 100%"  height="520px"  :header-cell-style="{textAlign:'center'}" :cell-style="{textAlign:'center'}">
          <!-- <el-table-column type="index"></el-table-column> -->
    ...
    <!-- 积分规则模态框 -->
    <el-dialog :visible.sync="dialogVisibleRule" :before-close="handleClose" class="reviseRule" :fullscreen='false' center :inline-message="true" status-icon width="30%">
      ...
      <div class="reviseContent">
        <div>
          1、用户在APP端或者教师版后台完善自己的信息得10积分;
        </div>
        <div>
          2、用户在APP端或者教师版后台 <span style="color:rgb(192,180,131);font-weight:600"> 发布作业、发布通知、发布新闻风采、发布班级圈</span>各得5积分;
        </div>
        <div>
          3、签到规则：连续签到第1-7天每天得5积分，连续签到第8-14天每天得6积分，连续签到第15-21天每天得8积分，连续签到第22-28天每天得13积分，第7/14/21/28天用户可以额外获得奖励积分，连续签到天数越多所获得的积分越多。以28天为一个周期循环。
        </div>
        <div>
    ...
    <!-- 兑换话费首先输入被充值的手机号码 -->
    <el-dialog :visible.sync="dialogVisiblMobileAndMobile" :before-close="handleClose" class="reviseMobileAndPw" :fullscreen='false' center :inline-message="true" top='20vh' status-icon width="30%">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogVisiblMobileAndMobile=false">取 消</el-button>
        <el-button type="primary" @click="pay()">下一步</el-button>
      </span>
    </el-dialog>    
    <!-- 兑换话费模态框 -->
    <el-dialog :visible.sync="dialogVisiblExchange" :before-close="handleClose" class="reviseExchange" :fullscreen='false' center :inline-message="true" status-icon width="30%">
      <div slot="title" class="reviseTitle">
        可兑换积分<span>{{bonusPoint}}</span>分
      </div>
      ...
      <div style="color:red;font-size:14px;padding-top:16px;text-align:center;" v-show="notice">您是为其他号码充值,需要使用双倍积分哟</div>
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogVisiblExchange=false">取 消</el-button>
    ...
    <!-- 输入密码模态框 -->
    <el-dialog :visible.sync="dialogVisiblMobileAndPw" :before-close="handleClose" class="reviseMobileAndPw" :fullscreen='false' center :inline-message="true" top='20vh' status-icon width="30%">
      <div slot="title" class="reviseTitle">
            请输入您的登录密码
      </div>
      <div class="reviseContent">
    ...
    <!-- 兑换成功模态框 -->
    <el-dialog :visible.sync="dialogVisiblOK" :before-close="handleClose" class="reviseOK" :fullscreen='false' center :inline-message="true" status-icon width="30%">
      <div class="reviseContent">
