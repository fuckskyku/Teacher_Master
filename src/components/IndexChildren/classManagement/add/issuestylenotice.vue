<template>
  <div id="issuestylenotice">
      ...
      <div class="stepcontainer">
        ...
        <div class="addthee">
          ...
          <el-upload class="avatar-uploader"
           action="/api/storage/uploadSingleFile" :multiple="false" :show-file-list="false"
            :on-success="handleAvatarSuccess" :headers="headers" 
            :before-upload="beforeAvatarUpload">
            <img v-if="styleObject.imageUrl" :src="this.imgJoInUrl() + styleObject.imageUrl" class="avatar">
      ...
      <div class="stepcontainer">
        ...
        <div class="addone">
          ...
          <el-row class="" v-if="radio">
            <el-scrollbar :native="false" class="scrollbar" :style="{height:styleObject.styleList.length>3?'100px':styleObject.styleList.length==0?'':'50px'}">
              <div v-for="(item,index) in styleObject.styleList" :key="index">
                ...
                <i v-if="styleObject.styleList.length-1!=index"><img src="/static/img/tip.png" alt=""></i>
              </div>
              <!-- <el-tag 
        ...
        <div class="addtwo">
          ...
          <el-row class="" v-if="radio1">
            <el-scrollbar :native="false" :style="{height:styleObject.copyList.length>3?'100px':styleObject.copyList.length==0?'':'50px'}">
              <el-tag v-for="(item,index) in styleObject.copyList" :key="index" type="warning" closable :disable-transitions="false" @close="handleClose(item.MarkId,index,'copy',item.flag)">{{item.label?item.label:item.teacherName?item.teacherName:item.realName?item.realName:''}}</el-tag>
            </el-scrollbar>
          </el-row>
    ...
    <div class="stepthree" v-if="issueShow==3?true:false">
      <div class="stepcontainer">
        ...
        <div class="addtwo" v-if="publishType==2">
          <div>网址链接:</div>
          <el-input v-model="styleObject.urlAddress" placeholder="请输入网址http://www.teacher.school.mseenet.com/"></el-input>
        </div>
        <div class="addtwo" v-if="publishType==1">
          <div>请编写内容:</div>
          <quill-editor v-model="styleObject.content" ref="myQuillEditor" :options="editorOption" @blur="onEditorBlur($event)" @focus="onEditorFocus($event)" @change="onEditorChange($event)">
          </quill-editor>
    ...
    <!-- 通知 -->
    <el-dialog :visible.sync="dialogTableVisible" :fullscreen="false" width="580px" top="30vh" :show-close=false>
      <div class="notice">
        <div>
        ...
        <el-row>
          <el-button round @click="okStyleOne">退出</el-button>
          <el-button round @click="okStyle">进入列表</el-button>
        </el-row>
      </div>
    </el-dialog>

    <!-- 通知 -->
    <el-dialog :visible.sync="dialogTableVisibleTwo" :fullscreen="false" width="580px" top="30vh" :show-close=false>
      <div class="notice">
        <div>
          <img src="/static/insetImg/插画/流程步骤/发布作业/发布成功.png" alt="">
        </div>
    ...
    <!-- 审批组织树架构 -->
    <el-dialog :visible.sync="dialogTableVisibleTree" :fullscreen="false" width="400" top="20vh" :center='true'>
      <div class="container">
        <el-scrollbar :native="false" tag="section">
          ...
          <el-tree :data="treeDateExamineAndExamine" :default-checked-keys="styleObject.styleIdsList" ref="TreeTwo" 
          :filter-node-method="TreefilterNodeExamine"
          :props="defaultProps" 
          :default-expand-all="true"
          @node-click='nodeClick' node-key="MarkId">
          </el-tree>
        </el-scrollbar>
      </div>
    </el-dialog>
    ...
    <el-dialog :visible.sync="dialogTableVisibleTreeTwo" :fullscreen="false" width="400" top="20vh" :center='true'>
      <div class="container">
        <el-scrollbar :native="false" tag="section">
          ...
          :default-checked-keys="styleObject.copyIdsList" ref="Tree" 
          :filter-node-method="TreefilterNodeCopy" :props="defaultProps" 
          show-checkbox
          @node-click='nodeClick' node-key="MarkId">
          </el-tree>
        </el-scrollbar>
    ...
    <el-dialog :visible.sync="dialogTableVisibleLook" :fullscreen="false" width="1200px" top="15vh" height='500px' :close-on-click-modal="false">
      <div slot="title" class="lookTitle">
        {{styleObject?styleObject.styleTitle:''}}
      </div>
      <el-scrollbar :native="false" tag="section">
        <div class="lookContent">
          ...
          <iframe :src="styleObject?styleObject.urlAddress:''" frameborder="0" width="100%" height="500px" v-if="styleObject?styleObject.urlAddress:''"></iframe>
          <!-- <div>{{styleObject}}</div> -->
        </div>
      </el-scrollbar>
    </el-dialog>
