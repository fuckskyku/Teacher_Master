<template>
  <div id="noticeinfo">
    <div class="stepone" v-if="issueShow==1?true:false">
      ...
      <div class="stepcontainer">
        ...
        <div class="addone">
          <div>类型:</div>
          <el-select
            v-model="noticeObject.noticeVal"
            placeholder="请选择"
            clearable
            @change="noticeChange"
          >
        ...
        <div class="addtwo">
          ...
          <div>
            <el-tag
              v-for="(item,index) in noticeObject.classList"
              :key="index"
              type="warning"
              closable
              :disable-transitions="false"
              @close="handleClose(item.classId,index,'class')"
            >{{item.label?item.label:item.teacherName}}</el-tag>
          </div>
          <div>
            <el-tag
              v-for="(item,index) in noticeObject.userList"
              :key="index"
              type="warning"
              closable
              :disable-transitions="false"
              @close="handleClose(item.MarkId,index,'user')"
            >{{item.label?item.label:item.teacherName}}</el-tag>
          </div>
        ...
        <div class="addone">
          ...
          <el-row class v-if="radio">
            <el-scrollbar
              :native="false"
              class="scrollbar"
              :style="{height:noticeObject.noticeList.length>3?'100px':noticeObject.noticeList.length==0?'':'50px'}"
            >
              <div v-for="(item,index) in noticeObject.noticeList" :key="index">
                ...
                <i v-if="noticeObject.noticeList.length-1!=index">
        ...
        <div class="addtwo">
          ...
          <el-row class v-if="radio1">
            <el-scrollbar
              :native="false"
              class="scrollbar"
              :style="{height:noticeObject.copyList.length>3?'100px':noticeObject.copyList.length==0?'':'50px'}"
            >
              <el-tag
                v-for="(item,index) in noticeObject.copyList"
                :key="index"
                type="warning"
                closable
                :disable-transitions="false"
    ...
    >
      <div class="container">
        ...
        <el-scrollbar :native="false" tag="section">
          <el-input placeholder="搜索家长姓名" v-model="TreefilterTextParent" style="margin-bottom:20px"></el-input>
          ...
          <el-tree
            ...
            :props="defaultProps"
            show-checkbox
            :default-checked-keys="noticeObject.classIdsList"
            node-key="id"
      ...
      <div class="container">
        <el-scrollbar :native="false" tag="section">
          <el-input placeholder="搜索教师姓名" v-model="TreefilterTextTeacher" style="margin-bottom:20px"></el-input>
          <div style="margin-bottom: 20px;">选择老师</div>
          <el-tree
            :data="treeDateExamineAndCopy"
            :default-checked-keys="noticeObject.userIdList"
            ref="TreeTeacher"
            :filter-node-method="TreefilterNode"
    ...
    <!-- 组织树架构审批 -->
    <el-dialog
      :visible.sync="dialogTableVisibleTreeTwoExamine"
      :fullscreen="false"
      width="400"
      top="20vh"
      :center="true"
    ...
    <el-dialog
      ...
      :close-on-click-modal="false"
    >
      <div slot="title" class="lookTitle">{{noticeObject?noticeObject.noticeName:''}}</div>
      <el-scrollbar :native="false" tag="section">
  ...
  methods: {
    ...
    handleClose(classId, index, type, flag) {
      if (type == "class") {
        ...
        this.$refs.TreeParent.setCheckedKeys(
          this.noticeObject.classIdsList,
          true
        );
      } else if (type == "user") {
        //   // this.dynamicTags.splice(index, 1)
        ...
        this.$refs.TreeTeacher.setCheckedKeys(
          this.noticeObject.userIdList,
          true
        );
      } else if (type == "examine") {
        if (!flag) {
                ...
                if (!this.queryObj.id) {
                  if (res.data.data.approvalUserList.length != 0) {
                    ...
                    this.noticeObject.noticeList =
                      res.data.data.approvalUserList;
                    res.data.data.approvalUserList.forEach(item => {
