 ...
 * -----
 * Last Modified: Monday, 21st January 2019 2:01:04 pm
 ...
 <template>
  <div id="PerformanceDivisionManagement">
      ...
      <el-table-column prop="score" label="分数" width="110">
        <template slot-scope="scope">
          <span
            ...
            :style="{color:scope.row.examStatus==0?'':'red'}"
          >{{scope.row.examStatus==0?scope.row.score:'缺考'}}</span>
          <el-input
            v-if="scope.row.hoverShow"
            type="number"
            min="0"
    ...
    <el-dialog :visible.sync="dialogVisible2" top="30vh" :close-on-click-modal="false" width="30%">
      ...
      >是否将{{tableData.data?tableData.data[0].subjectName:null}}成绩和评价内容发送给家？包括以下内容：</div>
