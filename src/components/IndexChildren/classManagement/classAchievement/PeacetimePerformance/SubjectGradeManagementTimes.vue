 <template>
  <div id="SubjectGradeManagement">
    ...
    <el-table :data="tableData.data" style="width: 100%" height="600" :header-cell-style="{textAlign:'center'}" :cell-style="{textAlign:'center',color:'#606266'}">
      ...
      <el-table-column prop="examName" label="平时成绩名称" width="150"></el-table-column>
      <el-table-column prop="examType" label="平时成绩类型" width="150"></el-table-column>
    ...
    <pageHtml :tabObj.sync='tableData' name="classStudentSubjectScoreClassSubjectScoreList" :otherType='queryObj' :discriminate='2'></pageHtml>
  </div>
</template>
<script>
import {
...
export default {
  ...
  methods: {
    exminationInfo(row) {
      this.$router.push({
        name: 'SubjectGradeManagementTimesInfo',
        query: {
          classId: row.classId,
          term: row.term,
