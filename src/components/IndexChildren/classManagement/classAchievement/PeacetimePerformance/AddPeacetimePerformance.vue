<template>
  ...
  <div id="AddTestScores">
    <div class="form">
      <el-form ref="ruleForm" :model="form" label-width="110px">
        <el-form-item
          label="平时成绩名称"
          prop="eaminationName"
          :rules="[{ required: true, message: '请输入平时成绩名称', trigger: 'change' }]"
        >
          <el-input
            v-model="form.eaminationName"
            clearable
            placeholder="请输入平时成绩名称(50字)"
            maxlength="50"
        ...
        <el-form-item
          label="平时成绩类型"
          prop="eaminationType"
          :rules="[{ required: true, message: '请选择平时成绩类型', trigger: 'change' }]"
        >
          <el-select
            v-model="form.eaminationType"
            placeholder="请选择平时成绩类型"
        ...
        <el-form-item label="考试成绩" class="tip-content">
          <el-upload
            ...
            :multiple="false"
            :data="{
                                  term:this.form.schoolTerm,
                                  examType: this.form.eaminationType,
                                  examName: this.form.eaminationName,
                                  type:2,
                                  examTime:this.form.eaminationTime}"
          >
    ...
    <el-dialog :visible.sync="dialogVisible" width="900px" :close-on-click-modal="false">
      <div slot="title" class="title">平时成绩类型管理</div>
      <div class="container">
        <div>
        ...
        <pageHtml :tabObj.sync="tableData" name="examTypeExamTypePage" :discriminate="2"></pageHtml>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      eaminationTypeObj: [],
      dialogVisible: false,
      form: { schoolTerm: "", eaminationName: "" },
      tableData: [],
      eaminationDelAllObj: [],
      fileFlag: false,
      fileFlagErr: false,
  ...
  methods: {
    examTypeExamTypeListFun() {
      //考试下拉列表
      examTypeExamTypeList({ type: 2 }).then(res => {
        if (res.data.code == 200) {
          console.log(res.data.data);
