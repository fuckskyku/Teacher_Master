  ...
  <div id="classcheck">
    <div class="seach">
      <div class="seach_left">
        <el-date-picker
          style="width:220px"
          v-model="statrAndEndVal.valueDateOne"
          type="date"
        ...
        <el-date-picker
          ...
          v-model="statrAndEndVal.valueDateTwo"
          type="date"
          value-format="yyyy-MM-dd"
          clearable
          @change="startAndEnd"
          :picker-options="{disabledDate(time) { return new Date(statrAndEndVal.valueDateOne)?time.getTime() < new Date(statrAndEndVal.valueDateOne).getTime():null;}}"
          placeholder="结束日期"
        ></el-date-picker>

        <!-- <el-date-picker
          v-model="statrAndEndVal"
          type="daterange"
      ...
      <div class="seach_right">
        <el-button round class="checkbutton" @click="leadingOut">EXCEL导出</el-button>
      </div>
    </div>
    <el-table
      :data="tableData.data"
    ...
    >
      ...
      <el-table-column prop="checkDate" label="日期">
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ new Date(scope.row.checkDate).format('yyyy-MM-dd')}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="isCheck" label="状态">
        <template slot-scope="scope">
          <span
            ...
            :class="[scope.row.isCheck==1?'bule':'red']"
...
export default {
  data() {
    return {
      tableData: [],
      statrAndEndVal: { valueDateOne: "", valueDateTwo: "" }
    };
  },
  methods: {
    //考勤数据导出
    ...
    startAndEnd(val) {
      ...
      classPage({
        startDate: this.statrAndEndVal.valueDateOne
          ? this.statrAndEndVal.valueDateOne
          : null,
        endDate: this.statrAndEndVal.valueDateTwo
          ? this.statrAndEndVal.valueDateTwo
          : null
      }).then(res => {
