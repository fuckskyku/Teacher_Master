  ...
  <div id="classFamily">
    <div class="seach">
      <div class="seach_left">
        <el-input placeholder="搜索家长姓名、学生姓名、手机号码(回车进行搜索)" suffix-icon="el-icon-search" clearable v-model="keywordsVal" @change="keyWordsInput"></el-input>

        <div class="section">
          <div class="section_title">筛选：</div>
          <el-select v-model="activatedStateVal" placeholder="激活状态" @change="activatedStateEvent" clearable>
            <el-option v-for="item in activatedStateObj" :key="item.dictCode" :label="item.dictName" :value="item.dictCode"></el-option>
          </el-select>
        </div>
      ...
      <div class="seach_right">
        <el-button icon="el-icon-bell" class="seach_yellow" round @click="parentsendBatchParentActivateNoticetAll">一键通知激活</el-button>
        <el-button icon="el-icon-delete" type="danger" round @click="batchDel">批量删除</el-button>
      </div>
    </div>

    <el-table :data="tableData.data" style="width: 100%" height="600" @select="checkbox" @select-all="checkboxAll" :header-cell-style="{textAlign:'center'}" :cell-style="{textAlign:'center',color:'#606266',padding:'4px 0'}">
      ...
      <el-table-column prop="kinshipId" label="关系">
        <template slot-scope="scope">
          ...
          <el-select v-model="scope.row.kinshipId" placeholder="请选择" v-if="scope.row.hoverShow" @change="kinshipIdChange($event,scope.row)">
            <el-option v-for="item in relationObj" :key="item.dictCode" :label="item.dictName" :value="item.dictCode"></el-option>
          </el-select>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="idStatus" label="是否激活">
        <template slot-scope="scope">
          <span :style="{'color':scope.row.idStatus==0?'#F56C6C':'#606266'}">{{scope.row.idStatus==0?'未激活':'已激活'}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="180" prop="idStatus">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.idStatus==0&&!scope.row.hoverShow" class="warning" type="text" @click="parentsendParentActivateNoticeEvent(scope.row)">通知激活</el-button>
          <el-button v-show="!scope.row.hoverShow" class="delete" type="text" @click="del(scope.row,scope.$index)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <pageHtml :tabObj.sync='tableData' name="parentPageList" :keywords='keywordsVal' :selectType="activatedStateVal" edit=true></pageHtml>
...
export default {
  ...
  methods: {
    ...
    //单个通知激活
    parentsendParentActivateNoticeEvent(row) {
      parentsendParentActivateNotice({
        id: row.id
      }).then(res => {
        if (res.data.code == 200) {
